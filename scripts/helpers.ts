...
export async function exitPool(
  ...
  if (approve) {
    ...
    for (let i = 0; i < tokens.length; i++) {
      ...
      console.log(`approving ${tokens[i]} ${initialBalances[i].toString()}`);

      const txn = await token.approve(vault.address, initialBalances[i]);
      await txn.wait();
      await delay(2500);
    }
  }
...
// }): Promise<void> {
//   const verifier = new Verifier(network, etherscanApiKey);
